...
#[derive(Clone, Debug, Serialize)] #[serde(rename_all = "camelCase")] pub struct Subscription { pub id: String, pub name: String, pub updated_at: String, pub server_count: usize }
#[derive(Clone, Debug, Serialize)] #[serde(tag = "kind", rename_all = "snake_case")] pub enum ConnectionState { Idle, Preparing, StartingCore, Connected, Stopping, Error { message: String } }
#[derive(Serialize)] #[serde(rename_all = "camelCase")] pub struct AppSnapshot { pub connection: ConnectionState, pub servers: Vec<ServerSummary>, pub subscriptions: Vec<Subscription>, pub selected_server_id: Option<String> }
